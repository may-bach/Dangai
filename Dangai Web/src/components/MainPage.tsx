...
const ArcCard: React.FC<ArcCardProps> = ({ arc, onSelect }) => {
    ...
    const cardClasses = `
        bg-white/5 border border-white/15 rounded-2xl px-8 text-center transition-all duration-300
        ${isPrologue ? 'py-8 flex items-center justify-center' : 'py-4'}
        ${arc.locked 
    ...
    return (
        <div className={cardClasses} onClick={() => !arc.locked && onSelect(arc)}>
            <div>
                <div className={`font-playfair text-xl md:text-2xl font-medium text-white tracking-[0.2em] md:tracking-[0.25em] [text-shadow:0_0_15px_rgba(255,255,255,0.4)] ${!isPrologue ? 'mb-4' : ''}`}>
                    {arc.title}
                </div>
                {!isPrologue && (
                    <div className={`font-serif text-base md:text-lg italic ${arc.locked ? 'text-neutral-500' : 'text-neutral-300'} ${arc.id === 'first_arc' ? 'tracking-[0.2em]' : ''}`}>
                        {arc.description}
                    </div>
                )}
            </div>
    ...
    return (
        <div className="container mx-auto max-w-4xl lg:max-w-5xl xl:max-w-6xl px-6 py-8 md:py-12 animate-fadeIn">
            <header className="text-center mb-16 md:mb-20">
                <h1 className="font-cinzel font-normal text-white [text-shadow:0_0_60px_rgba(255,255,255,0.8),0_0_150px_rgba(255,255,255,0.7)] text-4xl sm:text-5xl md:text-6xl lg:text-7xl xl:text-8xl tracking-[0.1em] sm:tracking-[0.15em] lg:tracking-[0.2em] mt-8 md:mt-10">
                    {NOVEL_TITLE}
                </h1>
            </header>
